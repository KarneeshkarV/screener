"""Generate LARGER training data for ML v5.

Expands universe + strategies + date range.
Smaller batch to complete within timeout.
"""
...
STRATEGIES = [
    ...
    # NEW: 20-day breakout (more frequent signals)
    ("breakout_20d", "close >= highest(close, 20) * 0.98 and volume > sma(volume, 10)", None, 10, 8, 0.05, 0.12, 0.04),
    # NEW: EMA pullback in uptrend
    ("ema_pullback", "close > ema(close, 50) and close < ema(close, 20) and close > ema(close, 20) * 0.97", "crossover(close, ema(close, 20))", 10, 8, 0.05, 0.12, 0.04),
]
...
def main():
    out_dir = Path(__file__).parent / "training_data_v5"
