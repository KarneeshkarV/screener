"""V5 Hyperparameter Optimization -- 100 iterations on EXPANDED dataset (14,900 trades)."""
...
def evaluate_config(X, y, markets, cfg, n_splits=5):
    if cfg.get("feature_subset"):
        use_cols = [c for c in X.columns if c in cfg["feature_subset"]]
    else:
    ...
    for fold, (train_idx, test_idx) in enumerate(skf.split(X_sub, stratify)):
        ...
        fold_top10_wr.append((y_test[sel10] > 0).mean())
        fold_top10_avg.append(y_test[sel10].mean())

        n20 = max(1, int(len(y_test) * 0.2))
        sel20 = sorted_idx[:n20]
        fold_top20_wr.append((y_test[sel20] > 0).mean())
        fold_top20_avg.append(y_test[sel20].mean())
...
def main():
    print("=" * 70)
    print("V5 HYPERPARAMETER OPTIMIZATION -- EXPANDED DATASET (100 ITERATIONS)")
    print("=" * 70)
    ...
    for i, cfg in enumerate(configs):
        ...
        metrics = evaluate_config(X, y, markets, cfg, n_splits=5)
    ...
    print("\n[4/4] Saving results...")
    results_dir = Path(__file__).parent / "training_data_v5"
    results_sorted = sorted(results, key=lambda r: r["composite_score"], reverse=True)

    output_path = results_dir / "optimization_results_v5_expanded.json"
    ...
    best_cfg = configs[results_sorted[0]["config_id"] - 1]
    use_cols = results_sorted[0]["feature_names"]
    ...
    model_path = results_dir / "model_v5_expanded_best.pkl"
    with open(model_path, "wb") as f:
        pickle.dump({
            "model": final_model,
            "feature_names": use_cols,
            "config": best_cfg,
            "metrics": results_sorted[0],
        }, f)
