"""Train ML Signal v4 binary model on large dataset."""
...
from screener.ml_signal_v4_binary import V4BinaryModel
...
def main() -> None:
    ...
    print("=" * 60)
    print("V4 BINARY MODEL — ALL MARKETS")
    print("=" * 60)
    model = V4BinaryModel()
    ...
    model.save(data_dir / "model_v4_binary.pkl")
    print(f"\nSaved to {data_dir / 'model_v4_binary.pkl'}")
    ...
    for market in ["us", "india"]:
        ...
        if len(m_trades) < 200:
            continue
        print(f"\n{'='*60}")
        print(f"V4 BINARY MODEL — {market.upper()}")
        print(f"{'='*60}")
        m_model = V4BinaryModel()
        ...
        m_model.save(data_dir / f"model_v4_binary_{market}.pkl")
        print(f"Saved to {data_dir / f'model_v4_binary_{market}.pkl'}")
