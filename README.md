...
- `--compounding` (`--no-compounding` to disable) grows each equal slot with realized equity so profits go back to work.
  ...
  It also sets the equity the risk rules size against; it has no effect under `--sizing reinvested_equal_slot`.
...
Their "equity" below is realized equity (cash plus the cost basis of open positions) under the default, and day-one `initial_capital` under `--no-compounding`, so that flag re-bases these four rules as well as the equal slot:
...
`reinvested_equal_slot` is the exception: it always sizes from marked-to-market equity and ignores `--compounding` entirely, which the run header reports as `compounding=n/a`.
