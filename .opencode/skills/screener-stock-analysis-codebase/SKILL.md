...
- Ranking scores are separate from filters and live in `screener/screener/scoring/`; output is always `setup_score`.
  There are two kinds of recipe and the kind must be declared, never inferred.
  Snapshot recipes read TradingView's per-row columns: add `@scorer("name", columns=(...), data_source=SNAPSHOT_SOURCE)` in `screener/screener/scoring/plugins/`, using the same name as the criterion.
  `data_source` is a required keyword with no default, so a plugin written without it raises at import time and takes the whole CLI down.
  Snapshot recipes are screen-only; `ensure_backtestable_scorer` rejects them in the backtest path because a snapshot carries only today's value and replaying it through history is lookahead.
  Bar-derived recipes read OHLCV instead: register the formula once with `@price_score("name", required_lookback=..., aux_column=..., eligible_above=...)` in `screener/screener/factors/recipes.py`, then expose it to the screen with `register_bar_scorer("criterion_name", "price_score_name")` in `screener/screener/scoring/plugins/technical.py`.
  Combined `-c` criteria equal-weight average their snapshot scorers via `resolve_scorer`, but `resolve_scorer` raises `IncompatibleScorerBlendError` (a CLI usage error) when any selected criterion is bar-derived, because a bar-derived 0-100 percentile is computed from price history over the scan's survivors and is not the same ranking as a snapshot composite.
  Screen bar-derived criteria one at a time.
- Shared price-only factor layer: `screener/screener/factors/` holds each bar-derived recipe exactly once.
...
- Stop distance: `screener/screener/backtester/stops.py`. `--stop-loss` is one flat fraction for every ticker; `--stop-atr N` (with `--stop-atr-window`, default 14) instead puts each position's stop `N x ATR` below its entry fill, measured at the signal bar, so a volatile name gets a wider stop than a quiet one. Both the exit sweep in `core.py` and the `atr_risk` sizer read the same function, so the size of a position and the stop that closes it cannot disagree; `--sizing atr_risk` therefore follows `--stop-atr` over `--sizing-atr-multiple` when both are set. A frame with no defined ATR at the signal bar falls back to `--stop-loss`.
