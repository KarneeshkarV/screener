...
- Intraday bars: `backtest-rolling` / `backtest-historical` accept `--interval` (`1d` default, `1h`, `30m`, `15m`, `5m`, `1m`); `build_price_fetcher(interval=..., market=...)` threads it through. yfinance caps history (1m ~30d, 5m-30m ~60d, 1h ~730d; no chunking yet), FMP serves intraday via `historical-chart` when `FMP_API_KEY` is set (raw, unadjusted bars, legacy flat cache). All intraday timestamps are canonical naive UTC across providers; yfinance intraday bars live in the interval-partitioned bar store `~/.screener/bars/{market}/{interval}/{symbol}.parquet` (daily bars keep the legacy flat cache). A stored 1m series serves 5m/15m/30m/1h requests via local session-anchored resampling; `screener bars record` (cron: `scripts/daily_bars_record.sh`) appends the trailing 1m window daily so the archive grows past the 30-day cap. Metrics annualize by bars-per-year for the interval; `--slippage-bps` defaults are interval-aware for intraday (2/3/5/7/10 bps at 1h/30m/15m/5m/1m, 0 at 1d). Long-warmup strategies (e.g. SMA200) usually cannot fill their lookback inside the capped intraday windows.
