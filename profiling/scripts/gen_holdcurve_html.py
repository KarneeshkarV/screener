...
def line_chart(series, ylabel, w=520, h=260, fmt="{:.2f}", zero=True):
    ...
    for g in range(5):
        ...
        y = yp(v)
        s.append(
            f'<line x1="{x0}" y1="{y:.1f}" x2="{x0 + pw}" y2="{y:.1f}" stroke="#262a33"/>'
        )
        s.append(
            f'<text x="{x0 - 6}" y="{y + 3:.1f}" fill="#7d8697" font-size="10" text-anchor="end">{fmt.format(v)}</text>'
        )
    if lo < 0 < hi:
        yz = yp(0)
        s.append(
            f'<line x1="{x0}" y1="{yz:.1f}" x2="{x0 + pw}" y2="{yz:.1f}" stroke="#4a5160" stroke-dasharray="3 3"/>'
        )
    # x labels
    for i, lab in enumerate(XLAB):
        x = xpos(i, x0, pw)
        s.append(
            f'<text x="{x:.1f}" y="{h - 10}" fill="#9aa4b2" font-size="10" text-anchor="middle">{lab}</text>'
        )
    # series
    for lab, col, vals in series:
        pts = [(xpos(i, x0, pw), yp(v)) for i, v in enumerate(vals) if v is not None]
        d = " ".join(
            f"{'M' if k == 0 else 'L'}{x:.1f} {y:.1f}" for k, (x, y) in enumerate(pts)
        )
        s.append(f'<path d="{d}" fill="none" stroke="{col}" stroke-width="2.2"/>')
        for x, y in pts:
            s.append(f'<circle cx="{x:.1f}" cy="{y:.1f}" r="2.6" fill="{col}"/>')
    s.append(
        f'<text x="{x0}" y="11" fill="#e6e6e6" font-size="11" font-weight="600">{ylabel}</text>'
    )
    s.append("</svg>")
    return "".join(s)
...
def mc_vals(m, key):
    return [MC[m][h][key] for h in HOLDS]


# ---- market-level charts ----
sharpe_series = [
    ("US avg Sharpe", US, mc_vals("us", "sharpe")),
    ("India avg Sharpe", IN, mc_vals("india", "sharpe")),
]
ret_series = [
    ("US avg Return %", US, mc_vals("us", "total_return")),
    ("India avg Return %", IN, mc_vals("india", "total_return")),
]
dd_series = [
    ("US avg MaxDD %", US, mc_vals("us", "max_dd")),
    ("India avg MaxDD %", IN, mc_vals("india", "max_dd")),
]
...
def strat_multiples(m):
    series = [
        (s, PAL[i], [SC[m][s][h]["sharpe"] for h in HOLDS])
        for i, s in enumerate(STRATS)
    ]
    return line_chart(
        series,
        f"{m.upper()} — Sharpe vs hold, per strategy",
        w=560,
        h=300,
        fmt="{:.2f}",
    ) + legend(series)
...
def mc_table(m):
    ...
    for h, lab in zip(HOLDS, XLAB):
        ...
        rows.append(
            f"<tr{cls}><td>{lab}{star}</td><td>{c['sharpe']:.3f}</td><td>{c['total_return']:+.1f}%</td>"
            f"<td>{c['cagr']:+.2f}%</td><td>{c['max_dd']:+.1f}%</td><td>{c['trades']:.0f}</td></tr>"
        )
    return (
        "<table><thead><tr><th>hold</th><th>avg Sharpe</th><th>avg Return</th>"
...
def bh_table(m):
    ...
    for s in STRATS:
        ...
        rr, rv = best_hold(m, s, "total_return")
        rows.append(
            f"<tr><td>{s}</td><td>{bh}</td><td>{bv:.3f}</td><td>{rr}</td><td>{rv:+.1f}%</td></tr>"
        )
    return (
        "<table><thead><tr><th>strategy</th><th>Sharpe-opt hold</th><th>Sharpe</th>"
        "<th>Return-opt hold</th><th>Return</th></tr></thead><tbody>"
        + "".join(rows)
        + "</tbody></table>"
    )
