...
STRAT_ORDER = ["mark_minervini","mq_us1","mq_us2","mq_us3","mq_in1","mq_in2","mq_in3"]
TOPS = ["5","10","20","40"]
HOLDS = ["5","10","20","30","40","60","100","inf"]
HCOLS = ["market","strategy","top","hold","sizing","total_return","cagr","sharpe",
         "max_dd","hit_rate","alpha","profit_factor","trades","bench_return"]
NCOLS = ["market","strategy","top","sizing","total_return","cagr","sharpe",
         "max_dd","hit_rate","alpha","profit_factor","trades","bench_return"]
METRICS = ("total_return","cagr","sharpe","max_dd","hit_rate","alpha","profit_factor","bench_return")

def num(s):
    if s is None: return None
    s = s.strip().replace("%","").replace("+","")
    if s in ("","ERROR","NO_TRADES"): return None
    try: return float(s)
    except: return None
...
for f in glob.glob(f"{RES}/*.csv"):
    ...
    if len(p) != len(HCOLS): continue
    r = dict(zip(HCOLS,p))
    for k in METRICS: r[k+"_n"] = num(r[k])
...
for line in open(f"{SP}/sweep_nohold_all.csv"):
    p = line.rstrip("\n").split(",")
    if len(p) != len(NCOLS): continue
    r = dict(zip(NCOLS,p))
    if r["sizing"] != "equal_slot": continue
    r["hold"] = "inf"
    for k in METRICS: r[k+"_n"] = num(r[k])
    r["trades_n"] = num(r["trades"])
...
for (m,s,t,h),r in data.items():
    if r["bench_return_n"] is not None: bench[m] = r["bench_return_n"]
...
for m in ("us","india"):
    market_curve[m] = {}
    for h in HOLDS:
        rows=[data[(m,s,t,h)] for s in STRAT_ORDER for t in TOPS if (m,s,t,h) in data]
        market_curve[m][h] = {
...
json.dump({"market_curve":market_curve,"strat_curve":strat_curve,"bench":bench,
           "holds":HOLDS,"strat_order":STRAT_ORDER},
          open(f"{SP}/holdcurve.json","w"), indent=2)
...
for m in ("us","india"):
    ...
    print(f"  {'hold':>5} | {'avgSharpe':>9} | {'avgRet%':>8} | {'avgCAGR%':>8} | {'avgMaxDD%':>9} | {'avgTrades':>9}")
    for h in HOLDS:
        c=market_curve[m][h]
        print(f"  {HL(h):>5} | {c['sharpe']:>9.3f} | {c['total_return']:>8.1f} | {c['cagr']:>8.2f} | {c['max_dd']:>9.1f} | {c['trades']:>9.0f}")
    ...
    print(f"  --> best avg Sharpe at hold={HL(bh)} ({market_curve[m][bh]['sharpe']:.3f});  best avg Return at hold={HL(br)} ({market_curve[m][br]['total_return']:.1f}%)")
...
for m in ("us","india"):
    ...
    for s in STRAT_ORDER:
        ...
        print(f"    {s:<14} Sharpe-opt hold={HL(bh):>4} (Sh {bv:.3f})   Return-opt hold={HL(bhr):>4} (Ret {bvr:.1f}%)")
