...
COLS=["mkt","strat","top","hold","tot","uniq","ratio","multi","pct_multi","mx","top3"]
...
data={}; namecount=defaultdict(Counter)
for f in glob.glob(f"{RES}/*.csv"):
    p=open(f).read().rstrip("\n").split(",",10)
    if len(p)<10: continue
    r=dict(zip(COLS,p))
    if r["tot"] in ("ERROR",""): continue
    for k in ("tot","uniq","multi","mx"): r[k]=int(r[k]) if r[k] else 0
    for k in ("ratio","pct_multi"): r[k]=float(r[k]) if r[k] else 0.0
    data[(r["mkt"],r["strat"],r["top"],r["hold"])]=r
    for tok in (r.get("top3") or "").split("|"):
        if ":" in tok:
            n,c=tok.rsplit(":",1)
            try: namecount[r["mkt"]][n]+=int(c)
            except: pass
...
for m in ("us","india"):
    curve[m]={}
    for h in HOLDS:
        rows=[data[(m,s,t,h)] for s in STRATS for t in TOPS if (m,s,t,h) in data]
        curve[m][h]={k:avg([r[k] for r in rows]) for k in ("ratio","pct_multi","tot","uniq","mx")}
...
def line_chart(title, series, ylabels_fmt="{:.2f}", w=520, h=260):
    ...
    def Y(v): return T+(B-T)*(hi-v)/(hi-lo)
    s=[f'<svg viewBox="0 0 {w} {h}" width="100%" style="max-width:{w}px">']
    for g in range(5):
        yv=lo+(hi-lo)*g/4; y=Y(yv)
        s.append(f'<line x1="{L}" y1="{y:.1f}" x2="{R}" y2="{y:.1f}" stroke="#262a33"/>')
        s.append(f'<text x="{L-6}" y="{y+3:.1f}" fill="#7d8697" font-size="10" text-anchor="end">{ylabels_fmt.format(yv)}</text>')
    for i,lab in enumerate(HL):
        s.append(f'<text x="{X(i):.1f}" y="{h-10}" fill="#9aa4b2" font-size="10" text-anchor="middle">{lab}</text>')
    for color,pts in series:
        ...
        for i,v in enumerate(pts):
            if v is not None: s.append(f'<circle cx="{X(i):.1f}" cy="{Y(v):.1f}" r="2.6" fill="{color}"/>')
    s.append(f'<text x="{L}" y="11" fill="#e6e6e6" font-size="11" font-weight="600">{title}</text></svg>')
    return "".join(s)
...
chart1=line_chart("Trades per unique name vs hold cap", [(US,ratio_us),(IN,ratio_in)], "{:.1f}")
chart2=line_chart("% of trades that are re-entries vs hold cap", [(US,pct_us),(IN,pct_in)], "{:.0f}")
legend='<div class="legend"><span><i style="background:%s"></i>US</span> <span><i style="background:%s"></i>India</span></div>'%(US,IN)
...
def mtable(m):
    ...
    return ('<table><tr><th>hold</th><th>trades/name</th><th>re-entry %</th>'
            '<th>trades</th><th>unique</th><th>max on 1 name</th></tr>'+rows+'</table>')

def names(m):
    return ", ".join(f'<code>{n.replace(".NS","")}</code>' for n,_ in namecount[m].most_common(8))
