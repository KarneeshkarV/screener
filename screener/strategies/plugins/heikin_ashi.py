...
def strat_heikin_ashi(df: pd.DataFrame) -> list[Trade]:
    ...
    for i in range(1, n_len):
        # Entry
        if (ha_open[i] > ha_close[i] and ha_open[i] == ha_high[i] and
            np.abs(ha_open[i] - ha_close[i]) > np.abs(ha_open[i-1] - ha_close[i-1]) and
        ...
        # Exit
        elif (ha_open[i] < ha_close[i] and ha_open[i] == ha_low[i] and 
              ha_open[i-1] < ha_close[i-1]):
