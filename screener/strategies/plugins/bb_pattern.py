...
import numpy as np
import pandas as pd
from screener.indicators.numpy import _bb, _stdev
        ...
        if not in_position:
            ...
            if cl[i] > upper[i]:
                ...
                for _j in range(i, i - period, -1):
                    # Entry condition 2
                    if (np.abs(middle[_j] - cl[_j]) < alpha) and (np.abs(middle[_j] - upper[i]) < alpha):
                        moveon = True
                        j = _j
                        break
                ...
                if moveon:
                    ...
                    for m in range(i, j, -1):
                        # Entry condition 3
                        if (cl[m] - lower[m] < alpha) and (cl[m] > lower[m]) and (cl[m] < threshold):
                            entries[i] = True
                            in_position = True
                            moveon = True
                            break
